...
def insert_sms_log(_sms_info):
    dbLog = torndb.Connection(config.GLOBAL_SETTINGS['log_db']['host'], config.GLOBAL_SETTINGS['log_db']
                              ['name'], config.GLOBAL_SETTINGS['log_db']['user'], config.GLOBAL_SETTINGS['log_db']['psw'])
    ...
    dbLog.insert(_sql, long(round(time.time() * 1000)) * 10000 + random.randint(0, 9999), 101,
...
def insert_register_log(_info):
    dbLog = torndb.Connection(config.GLOBAL_SETTINGS['log_db']['host'], config.GLOBAL_SETTINGS['log_db']
                              ['name'], config.GLOBAL_SETTINGS['log_db']['user'], config.GLOBAL_SETTINGS['log_db']['psw'])
    _sql = 'insert into log_async_generals (`id`,`logId`,`para01`,`para02`,`para03`,`para04`,`para05`,`para06`,`para07`,`para08`) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
    dbLog.insert(_sql, long(round(time.time() * 1000)) * 10000 + random.randint(0, 9999), 102,
...
def update_user_by_fee_info(_sms_cmd, _user):
    ...
    dbConfig = torndb.Connection(config.GLOBAL_SETTINGS['config_db']['host'], config.GLOBAL_SETTINGS['config_db']
                                 ['name'], config.GLOBAL_SETTINGS['config_db']['user'], config.GLOBAL_SETTINGS['config_db']['psw'])
    dbConfig.execute(_sql, _time_current,
                     _sms_cmd['price'], _sms_cmd['price'], _user['imsi'])
